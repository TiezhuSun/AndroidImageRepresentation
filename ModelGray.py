...
def parseargs():
    ...
    parser.add_argument("-t", "--type", help="The type of the image source files, which can only be one of 'dex', 'so' or 'xml'.", type=str, required=True)
...
dir_name = args.dir
assert args.type in ['dex', 'so', 'xml']
...
def get_image(path_img, mode):
    try:
        ...
        if mode == 'train':
            ...
            image = np.zeros((IMG_SIZE*IMG_SIZE, 1)).astype(np.uint8)
...
def process_test_path(file_path):
    label = get_label(file_path)
    img = decode_img(file_path, mode='test')
    return img, label
...
for i in range(1, 11):
    ...
    train_dataset = train_dataset.map(process_train_path, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    ...
    valid_dataset = valid_dataset.map(process_test_path, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    ...
    test_dataset = test_dataset.map(process_test_path, num_parallel_calls=tf.data.experimental.AUTOTUNE)
