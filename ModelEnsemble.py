...
from tensorflow.keras.layers import Flatten, Dense, Input, Lambda, Concatenate, Add, Dropout
import argparse
...
input_shape = (IMG_SIZE*IMG_SIZE, 1, 3)
...
def decode_img(path_img, mode):
    image = tf.numpy_function(get_image, [path_img, mode], tf.uint8)
...
def process_path(file_path):
    ...
    img = decode_img(file_path, mode='train')
    return img, label
...
def create_classifier():
    ...
    dex_channel = Lambda(lambda x: x[:, :, :, 0])(inputs)
    xml_channel = Lambda(lambda x: x[:, :, :, 1])(inputs)
    so_channel  = Lambda(lambda x: x[:, :, :, 2])(inputs)

    model_dex = keras.models.clone_model(model_architecture)
    model_dex._name = 'model_dex'
    dex_pre = model_dex(dex_channel)

    model_xml = keras.models.clone_model(model_architecture)
    model_xml._name = 'model_xml'
    xml_pre = model_xml(xml_channel)

    model_so = keras.models.clone_model(model_architecture)
    model_so._name = 'model_so'
    so_pre  = model_so(so_channel)

    combine_pre = Concatenate(axis=-1)([dex_pre, xml_pre, so_pre]) # (n_batch, n_feat, n_inputs)

    feature = Conv1D(filters=64, kernel_size=8, activation='relu')(combine_pre)
    feature = Flatten()(feature)
    feature = Dropout(0.1)(feature)

    feature = Dense(64, activation='sigmoid')(feature)
    prediction = Dense(1, activation='sigmoid')(feature)
...
for i in range(1, 11):
    ...
    es_callback = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=50, restore_best_weights=True)
